...
from sklearn.model_selection import train_test_split
from FL_BRNN.utils_fl import fl_convertion,reverse_normalization,reverse_standardation
import Config
from Config import brnn_config,fl_config
import matplotlib.pyplot as plt
import random
from FL_BRNN import brnn, federated_learning_nn
import gc
...
def brnn_imputation(x,y,start,timeSequence,opt,cols_orig):
    gc.collect()
    ...
    x_estimate = imputation(x,imputationd_value=-1)
    ...
    print(y_actual_)
    with open ('train_imputation.log','a') as f:
        f.write('-----------------x_train is-------------\n')
    ...
    if any(missing.isnull())==True:
        ...
        model_imputation,test_metrics=federated_learning_nn.eval(test_datasets,state,metrics)
                ...
                if np.isnan([missing[i][j]]) == True:
                    ...
                    if i<8:
                        ...
                        for iteration in range(5):  # Run 5 iterations
                            ...
                            pseudo_y_ = model_imputation.predict(x_train_fl_).astype(np.float32) # Generate pseudo-labels
                            ...
                            diff.append(diff_)
                            imputationd_value.append(missing_value)
                        with open ('imputation.txt','a') as f:
                            f.write(str(imputationd_value))
                            f.close()
                        with open ('diff.txt','a') as f:
                            f.write(str(diff))
                            f.close()
                        missing[i][j] = imputationd_value[np.argmin(diff)]
                    print(i,missing[i][j])
        del model_imputation
    ...
    x_imputation = missing_std
    print(x_imputation)
    df_imputation = pd.DataFrame(data=missing_std)
    df_imputation.to_csv('imputation.csv')
    

    return x_train,y_train,x_test,y_test,x_imputation,df_imputation
