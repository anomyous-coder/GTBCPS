...
from FL_BRNN import brnn
from FL_BRNN.brnn import neuralNetwork
import keras
import math
import argparse
import datetime
...
from FL_BRNN import federated_learning_nn
import gc
...
def FL_train_nn(x_train,y_train,x_test,y_test,y_actual,x_imputation,cols_orig,timeSequence,start,opt):
    gc.collect()
    ...
    x_imputation = scaler_predict.transform(x_imputation)
    ...
    #x_imputation = scaler.transform(x_imputation)
    #x_actual = x_imputation[trainSize:trainSize+testSize]
    ...
    for j in range(predictSize):
        ...
        x_actual1 = x_imputation[trainSize+j+1:trainSize+testSize+j]
        print(x_actual1)
    ...
    print(y_predict)
    del x_new
    #scale_y= scaler.fit(y_imputation)
